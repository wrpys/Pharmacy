...
public class GongyingshangDao {
	/*
	 * 把数据库查到的数据填充到Gongyingshang这个对象中，以便操作
	 * rs.getInt("gongyingshangID")
	 * rs是数据库查询的结果集
	 * getInt表示什么类型的数据
	...
	private static Gongyingshang converBusiness(ResultSet rs) throws Exception{
		...
		gongyingshang.setGongyingshangMingzi(rs.getString("gongyingshangMingzi"));
	...
	public static boolean save(Gongyingshang gongyingshang){
		try {
			...
			if(gongyingshang.getGongyingshangID()>0){
				//执行修改
				sql ="update gongyingshang set gongyingshangMingzi= '"+gongyingshang.getGongyingshangMingzi()+"',mingzi='"+gongyingshang.getMingzi()+"',dianhua='"+gongyingshang.getDianhua()+"' where gongyingshangID="+gongyingshang.getGongyingshangID();
			
			}else {
				//执行新增
				sql = "insert into gongyingshang(gongyingshangMingzi,mingzi,dianhua)";
				sql += " values('"+gongyingshang.getGongyingshangMingzi()+"','"+gongyingshang.getMingzi()+"','"+gongyingshang.getMingzi()+"')";
			...
			ProjectShare.log("gongyingshang.save/update error: "+e.getMessage());
			return false;
		}
	...
	public static boolean delete(int gongyingshangID){
		try {
			//开启数据库链接
			Connection connection = ProjectShare.getDbPool().getConnection();
			String sql = "delete from gongyingshang where gongyingshangID="+gongyingshangID;
			//表示数据库更新这个链接
...
//什么意思啊，被谁调用了这个方法？？？？？？？？？？？？？？？？？？？？？？？？？？？？
	public static Gongyingshang findByGongyingshangID(int gongyingshangID){
		...
		try {
			Connection connection = ProjectShare.getDbPool().getConnection();
			String sql = "select * from gongyingshang where gongyingshangID="+gongyingshangID;
			ResultSet rs = ProjectShare.getDbPool().query(connection, sql);
			if(rs.next()){
			...
			// TODO: handle exception
			ProjectShare.log("findBygongyingshangID gongyingshang error: "+e.getMessage());
			return null;
		}
	}
}
