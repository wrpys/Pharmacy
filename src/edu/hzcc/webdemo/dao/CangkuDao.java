...
/**
 * 仓库的数据库操作
 *以下方法都是由cangkuController调用，值返回给cangkuController
 */
public class CangkuDao {
	...
	public static boolean save(Cangku cangku){
		try {
			...
			if(cangku.getCangkuID()>0){
				//执行修改
				sql ="update cangku set cangkuMingzi= '"+cangku.getCangkuMingzi()+"' where cangkuID="+cangku.getCangkuID();
			}else {
				//执行新增
	...
	//caigoudingdan xiaoshoudingdan，cangkushezhi表没有仓库名字，只有cangkuID，用cangkuID获取cangku，再获取名字，返回到这三表的DAO，用于显示名字
